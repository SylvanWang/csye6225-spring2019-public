            ...
            s3.listBuckets(function(err, data) {
                if (err) {
                    ...
                    let promiseArray = files.map(f => {
                        let params = { Bucket: data.Buckets[1].Name, Key:`${Date.now()}-${f.originalname}`, Body: f.buffer };
                        console.log(params);
        ...
        if (process.env.NODE_ENV === 'dev') {
            ...
            s3.listBuckets(function(err, bucket) {
                if (err) {
                    ...
                    let params = {Bucket: bucket.Buckets[1].Name, Key: `${Date.now()}-${file.originalname}`, Body: file.buffer};
        ...
        if (process.env.NODE_ENV === 'dev') {
            s3.listBuckets(function(err, data) {
                if (err) {
                    ...
                    let params = {Bucket: data.Buckets[1].Name, Key: key};
